...
namespace WarehouseApp
{
    class DataGridViewPrinter
    {
        ...
        public String TruckText = null;
        public float TotalKg = 0;
        public int TotalCll = 0;
        public float TotalM3 = 0;
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            if (IsWithTitle)
            {
                ...
                //CurrentY += g.MeasureString(TheTitleText, TheTitleFont).Height + 50;
            }

            if (TruckText != null)
            {
                RectangleF rect = new RectangleF((float)(LeftMargin + g.MeasureString(TheTitleText, TheTitleFont).Width + 100), CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString("Truck", TheTitleFont).Height);
                g.DrawString("Truck: " + TruckText, TheTitleFont, new SolidBrush(TheTitleColor), rect, TitleFormat);
            }

            RectangleF rectangle = new RectangleF((float)(LeftMargin + g.MeasureString(TheTitleText, TheTitleFont).Width + g.MeasureString("TRUCK: 123456789", TheTitleFont).Width + 200), CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString("Date", TheTitleFont).Height);
            g.DrawString("Date: ", TheTitleFont, new SolidBrush(TheTitleColor), rectangle, TitleFormat);
            rectangle = new RectangleF(rectangle.X + g.MeasureString("Date: ", TheTitleFont).Width, rectangle.Y, rectangle.Width, rectangle.Height);
            g.DrawString(DateTime.Now.ToString("dd.MM.yyyy HH:mm"), new Font(TheTitleFont, FontStyle.Regular), new SolidBrush(TheTitleColor), rectangle, TitleFormat);
            rectangle = new RectangleF(TitleRectangle.X, TitleRectangle.Y + g.MeasureString("Total:", TheTitleFont).Height + 5, TitleRectangle.Width, TitleRectangle.Height);
            if (TotalM3 + TotalKg + TotalKg > 0)
            {
                g.DrawString("Total:", TheTitleFont, new SolidBrush(TheTitleColor), rectangle, TitleFormat);
                rectangle = new RectangleF(rectangle.X + g.MeasureString("Total: ", TheTitleFont).Width, rectangle.Y, rectangle.Width, rectangle.Height);
                if (TotalCll > 0)
                    g.DrawString(TotalCll + " Pkgs", TheTitleFont, new SolidBrush(TheTitleColor), rectangle, TitleFormat);
                rectangle = new RectangleF(rectangle.X + g.MeasureString(TotalCll + " Pkgs", TheTitleFont).Width + 30, rectangle.Y, rectangle.Width, rectangle.Height);
                if (TotalKg > 0)
                    g.DrawString(TotalKg + " Kgs", TheTitleFont, new SolidBrush(TheTitleColor), rectangle, TitleFormat);
                rectangle = new RectangleF(rectangle.X + 538, rectangle.Y, rectangle.Width, rectangle.Height);
                if (TotalM3 > 0)
                    g.DrawString(TotalM3 + " M3", TheTitleFont, new SolidBrush(TheTitleColor), rectangle, TitleFormat);
            }
            CurrentY += g.MeasureString(TheTitleText, TheTitleFont).Height + 50;
